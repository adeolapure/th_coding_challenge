...
app = flask.Flask('git_profile_api')
...
@app.route('/v1/git-metrics', methods=['GET'])
def get_git_metrics():
...
def parse_username_argument(request_args, username_key, profile_cls):
    """Initializes and validates :cls:`app.profiles.profiles.GitProfileSummary` implementations based
    ...
    :param username_key: The request argument parameter name from which values should be parsed and used
        to initialize a corresponding :cls:`~app.profiles.profiles.GitProfileSummary` object
    :type username_key: str
    :param profile_cls: The :cls:`app.profiles.profiles.GitProfileSummary` implementation type from which objects
    ...
    :rtype: list[app.profiles.profiles.GitProfileSummary]
    """
